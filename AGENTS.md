...
- 必須サービスは PostgreSQL / Redis / FFmpeg（Meilisearch は任意だが一部機能とテストで必要）。
- 初回セットアップは `pnpm install --frozen-lockfile`、`pnpm build`、`pnpm migrate` の順で実行する。
- 開発起動は `pnpm dev`。パッケージ名はルートと各 workspace の `package.json` を参照する。
- 変更したコードにはテストを追加または更新する。
- コミット前に `pnpm lint` と `pnpm test` を実行する。
